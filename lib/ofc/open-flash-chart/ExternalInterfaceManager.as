package 
{
	...
	public class ExternalInterfaceManager
	{
		...
		public function callJavascript(functionName:String, ... optionalArgs:*):*
		{
			...
			{
				tr.aces('External JS Function call: ' + functionName);
				
				var collectArgs:Array = new Array;
				collectArgs.push(functionName);
				
				if(this.has_id)
				{
					collectArgs.push(this.has_id);
				}
				
				for(var i:uint = 0; i < optionalArgs.length; i++)
				{
					collectArgs.push(optionalArgs[i]);
				}
				...
				*/
				...
				var callFunction:Function = ExternalInterface.call;
				return callFunction.apply(ExternalInterface, collectArgs);
