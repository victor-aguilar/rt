...
//			this.Elements= new Array();
		...
		private function add_slice( index:Number, start:Number, angle:Number, value:Object, tip:String, colour:String ): PieSliceContainer {
			...
			if ( value is Number )
				...
				tmp = value;
			if ((tmp['on-click-text'] == null) && (this.style['on-click-text'] != null)) {
				tmp['on-click-text'] = this.style['on-click-text'].replace('#total#', NumberUtils.formatNumber( this.total_value ));
				tmp['on-click-text'] = tmp['on-click-text'].replace('#percent#', NumberUtils.formatNumber( value is Number ? value as Number : value.value / this.total_value * 100 ) + '%');
			}
			if (tmp['key-on-click'] == null) {
				tmp['key-on-click'] = this.style['key-on-click'];
			}
			if (tmp['on-click-window'] == null) {
				tmp['on-click-window'] = this.style['on-click-window'];
			}				
			...
			var slice:PieSliceContainer = new PieSliceContainer( index, p );
			if (tmp.text != null) {
				this.keys.push( { 	'text':tmp.text, 
									'font-size':tmp['font-size'], 
									'colour':slice.get_colour(),
									'on-click':tmp['key-on-click'],
									'series':slice,
									'visibility-id': tmp.ID } );
			}
            return slice;
		}
		//
