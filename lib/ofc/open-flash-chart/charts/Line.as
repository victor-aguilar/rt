package charts {
	...
	public class Line extends Base
	{
		...
		public function solid_line(): void {
			...
			var first_real:Boolean = true;
			var gap:Boolean = true;
			var i:Number;
			var tmp:Sprite;
			var x:Number;
			var y:Number;
			var nullGap:Boolean = (this.props.get('null-gap') == true);
			//trace("null-gap prop " + this.props.get('null-gap'));
			//trace("null-gap " + nullGap);
			if (nullGap)
			{
				for ( i=0; i < this.numChildren; i++ ) {
					...
					if( tmp is Element )
					{
						...
						//trace(e._y);						
						//trace(e.realValue);						
						//trace(this.props.get('realValue'));						
						...
						else
						{
							...
							{
								gap = false;
								//Set as first part of line following null
								if (nullGap)
								{
									first = true;
								}
						...
						//trace('i ' + i);
						//trace('real ' + e.realValue);
						//trace('gap ' + gap);
						//trace('first ' + first);
						//trace('x,y ' + e.x + " , " + e.y);
						//if first point on line or line section
						if( first )
						{
							//trace('in first');
							//if current value is not null; first point of new line section
							if (!gap)
							{
								//trace('move to');
							...
							//if we are not leaving gaps then set current x y position
							if (!nullGap)
							{
								x = e.x;
								y = e.y;
			...
			else
			{
				...
				for ( i=0; i < this.numChildren; i++ ) {
					...
					if( tmp is Element )
					{
						...
						//trace('in first');
						//if current value is not null; first point of new line section
						if (e.realValue == 'null' || first == true)
						{
