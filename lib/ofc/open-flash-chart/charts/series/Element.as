package charts.series {
	...
	public class Element extends Sprite implements has_tooltip {
		...
		public function mouseOut(event:Event):void {
			...
			Tweener.removeTweens(this, "_alpha");
		...
		private function mouseUp(event:Event):void
		{
			if(this.link.substring(0, 6) == 'trace:')
			{
				tr.ace(this.link);
					...
					this.link.substring(0, 6) == 'https:' || 
					this.link.substring(0, 4) == 'ftp:' || 
					this.link.substring(0, 5) == 'sftp:' || 
					this.link.substring(0, 1) == '/' ||
					this.link.substring(0, 2) == './' ||
					this.link.substring(0, 3) == '../'
					)
			{
		 ...
		 * position is passed as a string to aviod errors with 0
		 * 
		 * @param	labels
		 * @param	position
		 */
		public function tooltip_replace_labels( labels:XAxisLabels ):void {
				//tr.aces('x label', this._x, this.get_x(), this.index ,labels.get( this.index ));
