package charts.series.bars {
	...
	public class Horizontal extends Element
	{
		...
		public function Horizontal( index:Number, style:Object, group:Number )
		{
			...
			trace(style['on-click']);
			if (style['on-click'] != undefined)
			{
				trace('defined');
				if ( style['on-click'] != false )
				{
					trace('set');
		...
		//	if ( style.has('on-click') )	// <-- may be null/not set
		//		trace('2');
		//		if ( style.get('on-click') != false )	// <-- may be FALSE
		//			trace(style.get('on-click'));
