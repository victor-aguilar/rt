...
package  {
	...
	public class main extends Sprite {
		...
		public  var VERSION:String = "2-Community 0.25";
		...
		public function main() {
			...
			if( !this.find_data() )
			{
				...
				try {
					var file:String = "../data-files/test.txt";
					//trace(file);
	    ...
	    private function image_binary() : ByteArray {
			...
			var tooltipStatus:Number = this.tooltip.get_tip_style();
			this.tooltip.set_tip_style( Tooltip.DISABLE );
			this.tooltip.hide_quick();
			var pngSource:BitmapData = new BitmapData(this.stage.stageWidth, this.stage.stageHeight);
 			pngSource.draw(this);
			this.tooltip.set_tip_style( tooltipStatus );
		...
		public function find_data(): Boolean {
			...
			else if( this.chart_parameters['data_file'] )
			{
				// tr.ace( 'Found parameter:' + parameters['data_file'] );
				this.load_external_file( this.chart_parameters['data_file'] );
				//
				// LOOK:
				//
				return true;
		...
		private function resize_pie(): ScreenCoordsBase {
			...
			this.sc = new ScreenCoords(top, 0, right, this.stage.stageHeight, null, null, null, 0, 0, false, this.stage.stageWidth);
			this.keys.resize(sc);
			this.obs.resize( sc );
		...
		private function parse_json( json_string:String ):void {
			...
			if(ok)
			{
				...
				// Callback for data loaded (loading data may take a while if dataset is dynamic and large)
				if(this.chart_parameters['id'])
				{
					this.callExternalCallback('ofc_data_ready', this.chart_parameters['id']);
				}
				else
				{
					this.callExternalCallback('ofc_data_ready');
				}
