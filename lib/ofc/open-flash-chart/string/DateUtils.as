package string
{
	public class DateUtils
	{
		...
		{
			// Convert from a unix timestamp to an AS3 date in seconds or milliseconds
			var as3Date:Date 		= new Date(xVal * 1000);
			var milliAs3Date:Date 	= new Date(xVal);
			//trace(xVal);
			//trace(milliAs3Date);
			tip = tip.replace('#date#', DateUtils.formatDate(as3Date, 'Y-m-d'));
			tip = tip.replace('#millidate#', DateUtils.formatDate(milliAs3Date, 'Y-m-d'));
			
			// Convert user formatted dates
			...
			// Convert user UTC formatted dates
			...
			// Convert user millisecond formatted dates
			begPtr = tip.indexOf("#millidate:");
			while(begPtr >= 0)
			{
				endPtr 					= tip.indexOf("#", begPtr + 1) + 1;
				replaceStr 				= tip.substr(begPtr, endPtr - begPtr);
				timeFmt 				= replaceStr.substr(11, replaceStr.length - 12);
				dateStr 				= DateUtils.formatDate(milliAs3Date, timeFmt);
				//trace(milliAs3Date);
				tip 	= tip.replace(replaceStr, dateStr);
				begPtr 	= tip.indexOf("#millidate:");
			}
